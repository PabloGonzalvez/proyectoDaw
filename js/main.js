...
function modificar_perfil(){
  ...
  xhttp.open("POST", "php/modificar_perfil.php", false);
  let formData = new FormData();
    
  let nombre= document.getElementById("mod_name").value;

  let bio=document.getElementById("mod_bio").value;

  let archivo=document.getElementById("mod_file");
...
function cargarUserData() {
  ...
  document.getElementById("userprofile_id").innerHTML=datosUsuario["usuario"];

  if(datosUsuario["nombre"]!=null){
    document.getElementById("userprofile_name").innerHTML=datosUsuario["nombre"];
  }

  if(datosUsuario["info"]!=null){
    document.getElementById("userprofile_bio").innerHTML=datosUsuario["info"];
  }

  let imagen;

  if(datosUsuario["imagen"]!=null){
    imagen='usuarios/'+usuarioId+'/'+datosUsuario["imagen"];
  }
  else{
    imagen='usuarios/default/avatar_2x.png';

  }
  document.getElementById("userprofile_image").setAttribute("src",imagen)
...
function cargar_post() {
  ...
  xhttp.open("POST", "php/cargar_post.php", true);
  let formData = new FormData();
  formData.append("usuario",usuarioId);
  xhttp.send(formData);
  return false;
...
window.onload = function () {
  ...
  document.getElementById("modificar_perfil").addEventListener("click", modificar_perfil);
  if (usuarioLogueado != null) {
    ...
    //Si hay usuario logueado mostramos contenido de usuario
